...
describe 'ppm link/unlink', ->
  ...
  describe "when the all flag is true", ->
    it "unlinks all packages in $ATOM_HOME/packages and $ATOM_HOME/dev/packages", ->
      ...
      runs ->
        ppm.run(['link', '--dev', packageToLink1], callback)
      ...
      runs ->
        callback.reset()
        ppm.run(['link', packageToLink2], callback)
        ppm.run(['link', packageToLink3], callback)
  ...
  describe "when the package name is numeric", ->
    it "still links and unlinks normally", ->
      ...
      runs ->
        ppm.run(['link', numericPackageName], callback)
      ...
      runs ->
        ...
        callback.reset()
        ppm.run(['unlink', numericPackageName], callback)
