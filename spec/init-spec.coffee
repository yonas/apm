...
describe "ppm init", ->
  ...
  describe "when creating a package", ->
    describe "when package syntax is CoffeeScript", ->
      it "generates the proper file structure", ->
        ...
        ppm.run(['init', '--package', 'fake-package'], callback)
    ...
    describe "when package syntax is JavaScript", ->
      it "generates the proper file structure", ->
        callback = jasmine.createSpy('callback')
        ppm.run(['init', '--syntax', 'javascript', '--package', 'fake-package'], callback)
    ...
    describe "when package syntax is unsupported", ->
      it "logs an error", ->
        callback = jasmine.createSpy('callback')
        ppm.run(['init', '--syntax', 'something-unsupported', '--package', 'fake-package'], callback)
    ...
    describe "when converting a TextMate bundle", ->
      beforeEach ->
        ...
        ppm.run(['init', '--package', 'fake-package', '--convert', textMateBundlePath], callback)
  ...
  describe "when creating a theme", ->
    it "generates the proper file structure", ->
      callback = jasmine.createSpy('callback')
      ppm.run(['init', '--theme', 'fake-theme'], callback)
    ...
    describe "when converting a TextMate theme", ->
      it "generates the proper file structure", ->
        ...
        ppm.run(['init', '--theme', 'fake-theme', '--convert', textMateThemePath], callback)
  ...
  describe "when creating a language", ->
    it "generates the proper file structure", ->
      callback = jasmine.createSpy('callback')
      ppm.run(['init', '--language', 'fake'], callback)
    ...
    it "does not add language prefix to name if already present", ->
      callback = jasmine.createSpy('callback')
      ppm.run(['init', '--language', 'language-fake'], callback)
